...
## Project Overview
A real-time multiplayer word-guessing game server built with Socket.IO and Express. Players take turns being the "guesser" while others write clues for a secret word. The game includes category selection, clue writing, voting to eliminate duplicates, and guessing phases.
...
├── data/
│   └── data.js               # Game categories and secret words
├── handlers/
...
├── defaultGameConstants.js   # Default game data
...
### Game Phases
1. **chooseCategory**: Guesser selects from available categories
...
- `chooseCategory`: Guesser selects category
...
- **Constants**: camelCase exports (`categories`, `secretWords`)
